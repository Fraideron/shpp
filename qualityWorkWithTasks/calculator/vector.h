...
*/
    ...
    /**
     * @brief insert - insert element in spec. position
     * @param position 
     * @param value
     * @return 
     */
    bool insert(int position, V value);

    /**
     * @brief erase - remove element in spec. position
     * @param position
     * @return 
     */
    ...
    /**
     * @brief erase - remove elements from positionStart to positionEnd
     * @param positionStart
     * @param positionEnd
     * @return 
     */
    bool erase(int positionStart, int positionEnd);
...
template <typename V>
bool vector<V>::insert(int position, V value){
    if(position == sizeVectr){
        push_back(value);
        return true;
    }

    if((position < 0) || (position > sizeVectr+1)){
        return false;
    }

    if((position >= 0) && (position < sizeVectr)){
        sizeVectr++;
        ...
        for (int i = sizeVectr; i > position; --i) {
            mainStorage[i] = mainStorage[i-1];
        }

        mainStorage[position] = value;
        return true;
    }
    return false;
}
...
template <typename V>
bool vector<V>::erase(int position){
    if(position == sizeVectr-1){
        pop_back();
        return true;
    }
    ...
    if((position >= 0) && (position < sizeVectr-1)){
        ...
        for (int i = position; i < sizeVectr-1; ++i) {
            mainStorage[i] = mainStorage[i+1];
        }
...
template <typename V>
bool vector<V>::erase(int positionStart, int positionEnd){
    ...
    if((positionStart >= 0) && (positionEnd < sizeVectr-1)){
        for (int i = 0; positionEnd + i < sizeVectr-1; ++i) {
            delete &mainStorage[positionEnd+i];
            mainStorage[positionStart+i] = mainStorage[positionEnd+i+1];
        }

        sizeVectr -= (positionEnd-positionStart)+1;
...
vector<V>& vector<V>::operator=(vector<V>& rhs) {
    ...
    this->sizeVectr= rhs.sizeVectr;
    for (int i = 0; i < rhs.sizeVectr; ++i) {
        this->mainStorage[i] = rhs.mainStorage[i];
    }
    return *this;
}
